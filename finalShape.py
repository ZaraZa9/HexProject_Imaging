...
#sample = cv2.imread('testMasks/triangleMask.png')
...
while rotation_angle != 360:
    ...
    height, width = extended_canvas.shape[0], extended_canvas.shape[1]
    center = (width // 2, height // 2)

    # Perform the rotation
    rotation_matrix = cv2.getRotationMatrix2D(center, rotation_angle, 1.0)
    rotated_image = cv2.warpAffine(extended_canvas, rotation_matrix, (width, height))
    ...
    coverage_percentage = numpy.sum(result_image != 0) / numpy.prod(focusedImage.shape) * 100
